...
function StyleSelector({styles, setSelectedStyle, resetIndex}) {
      ...
      <div>Selected style: {selected.name}</div>
      {
        styles.results.map((style, i) => (
          <div key={i} className="thumbnail-img">
            <img onClick={() => {setSelected(style); setSelectedStyle(style); resetIndex(0)}} src={style.photos[0].thumbnail_url}
