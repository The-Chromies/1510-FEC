...
function Overview({ goToReviews, getStyles, styles }) {
  ...
  useEffect(() => {
    styles && setSelected(styles.results[0]);
  }, [styles]);
    ...
    <Container>
      <Row className="overview-container">
        <Col xs={12} s={12} md={6} lg={8}>
          { styles && selected ? <ImageGallery className="image-gallery" selected={selected}/> : null }
        </Col>
